...
<header class="border">
	<div class="mx-auto flex items-center justify-between p-4 xl:container lg:px-16 xl:px-20">
		<nav class="flex gap-6 items-end">
			<a href="/" class="mr-6">
			...
			<div class="hidden md:block space-x-6">
				...
				<a href="mailto:hi@ryanmartin.me">Contact</a>
			</div>
		</nav>
		<nav class="hidden md:flex gap-3">
			<a href="/dashboard" class="btn"> Sign in </a>
			<a href="mailto:hi@ryanmartin.me" class="btn btn-primary"> Get in touch </a>
		</nav>
		<button on:click={() => (isOpen = !isOpen)} class="btn btn-icon md:hidden">
			{#if !isOpen}
				<svg
...
<nav class="md:hidden p-4 flex flex-col gap-2" class:hidden={!isOpen}>
	<a href="/" class="px-4 py-1 hover:bg-stone-100">Product</a>
