...
export const actions = {
  default: async ({ request, locals }) => {
    ...
    const parentBatches = body.parents.toString().split(',').map((e) => Number(e.trim()))
    ...
    if (parentBatches) {
      ...
      const { error: err5 } = await locals.supabase
        ...
        .insert(parents.map((e) => ({ parent_id: e.eliability_id, child_id: eliability.eliability_id })))
      ...
      if (!body.moved) {
        const { data: emissions, error: err6 } = await locals.supabase
          ...
          .in('eliability_id', parents.map((e) => e.eliability_id))
        ...
        const new_total_emissions = total_emissions + emissions.reduce((acc, e) => acc + e.total_emissions, 0)
